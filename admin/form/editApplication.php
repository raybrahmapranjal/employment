...
if (isset($_POST["editSubmit"])) {
    ...
    if (
        ...
        empty($_FILES["pdf1"]["name"]) &&
        empty($_FILES["pdf2"]["name"]) &&
        empty($_FILES["pdf3"]["name"]) &&
        empty($_FILES["pdf4"]["name"])
    ) {
        ...
        $count = $crud->Count("emp_application", "`application_ID`='$app_ID'");
        if ($count == 1) {
            $update = $crud->Update(
                "emp_application",
                $data,
        ...
        $target_file_image =
            $target_dir . md5(time()) . basename($_FILES["image"]["name"]);
        $imageFileType = strtolower(
            pathinfo($target_file_image, PATHINFO_EXTENSION)
        );
        ...
        for ($i = 1; $i <= 4; $i++) {
            $target_file_pdf =
                $target_dir .
                md5(time()) .
                basename($_FILES["pdf{$i}"]["name"]);
            $pdfFileType = strtolower(
                pathinfo($target_file_pdf, PATHINFO_EXTENSION)
            );
        ...
        if ($uploadOk) {
            ...
            $_SESSION["message"] =
                '<script>alert("Error uploading files!");</script>';
            echo '<script>window.location.assign("../application.php");</script>';
        }
    }
}
