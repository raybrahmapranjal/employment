...
if(isset($_POST['submit'])){
    ...
    if (empty($_FILES["image"]["name"])) {
        ...
        $count= $crud ->Count("emp_photo","1");
       
        if($count<1){
            $create = $crud->Create($data,"emp_photo");

            if($create) {
                $_SESSION['message']= '<script>alert("Data Uploaded!");</script>';
                echo '<script>window.location.assign("../view_photos.php");</script>';
            }
        } else {
        ...
        // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], "../".$target_file)) {
                ...
                if($count<1){
                    ...
                    $update =$crud->Create($data,"emp_photo");
