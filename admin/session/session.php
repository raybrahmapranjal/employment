...
date_default_timezone_set('Asia/Kolkata');
	...
	$stmt=$con->prepare("SELECT * FROM `emp_admin` WHERE (`email`='$email' || `username` = '$email') AND `password`='$hash'");
	...
	{	
		if ($row['status'] == 0) {
			...
			$sql2 = $con -> prepare("INSERT INTO `emp_admin_log_details` (`user_id`, `os`, `browser`, `ip`, `login_date`, `login_time`) VALUES ('$user_id', '$os', '$browser', '$ip', '$login_date', '$login_time')");
